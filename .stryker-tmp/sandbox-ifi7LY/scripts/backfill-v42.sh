#!/bin/bash
# ============================================================
# backfill-v42.sh — V42 invoices 表 + contracts.invoice_issued 全租户 ADD
#
# 来源：用户 2026-05-14 Wave 4 P0-2 finance-invoices/new 后端落地
#
# 模式：bash 循环 tenant_ids + sed __TENANT_SCHEMA__ apply migration
#       （参考 V33/V34/V35/V36/V37/V39 模式 — 与 V41 同型；V42 无 ts-node 数据 backfill）
#
# 与 V41 关键差异：
#   - V41 是「ADD COLUMN + UPDATE 加密数据」需 ts-node 单 tenant 数据回填
#   - V42 是「CREATE TABLE + ADD COLUMN DEFAULT FALSE」纯 schema 操作，无数据 backfill
#   - 仅跑 Phase 1（sed + psql apply），无 Phase 2
#
# 用法（生产服务器）：
#   # 1. dry-run（默认）— 列将处理的 tenant
#   bash scripts/backfill-v42.sh
#
#   # 2. 真执行
#   bash scripts/backfill-v42.sh --apply
#
#   # 3. 跳过备份检查（DANGEROUS，仅紧急回退用）
#   bash scripts/backfill-v42.sh --apply --skip-backup-check
#
#   # 4. 只跑指定 tenant
#   bash scripts/backfill-v42.sh --apply --tenant-id=01abcd...
#
# 前置：
#   - 在 PG 主机上执行（sudo -u postgres psql 本机连接）
#   - PG_DB env 默认 'edu'，可覆盖
#   - V42__invoices_in_tenant_schema.sql 文件存在
#
# 幂等：
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V42__invoices_in_tenant_schema.sql"
...
echo ""
echo "==============================================="
echo "  V42 invoices 表 + contracts.invoice_issued"
echo "==============================================="
echo ""
if [ "$APPLY" = false ]; then
  warn "DRY-RUN mode（默认）— 加 --apply 才会真执行"
else
  info "APPLY mode — 真执行（CREATE TABLE invoices + ADD COLUMN contracts.invoice_issued）"
fi
...
if [ "$SKIP_BACKUP_CHECK" = true ]; then
  ...
  if [ "$BACKUP_FOUND" = false ]; then
    ...
    warn "V42 是 CREATE TABLE + ADD COLUMN（可逆），缺备份不阻塞，但建议先跑备份"
    if [ "$APPLY" = true ]; then
...
while IFS='|' read -r TENANT_ID TENANT_NAME; do
  ...
  if [ "$APPLY" = false ]; then
    info "[dry-run] would CREATE TABLE invoices + ADD COLUMN contracts.invoice_issued on ${TENANT_SCHEMA} (sql size: $(wc -c < "$TMP_SQL") bytes)"
    SCHEMA_OK=$((SCHEMA_OK + 1))
  else
    if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" >/dev/null 2>&1; then
      ok "V42 applied to ${TENANT_SCHEMA}"
      SCHEMA_OK=$((SCHEMA_OK + 1))
    else
      fail "V42 failed on ${TENANT_SCHEMA}"
...
if [ $SCHEMA_FAILED -gt 0 ]; then
  fail "部分 tenant V42 失败（schema_failed=$SCHEMA_FAILED）"
  exit 1
fi

ok "全部 tenant V42 CREATE TABLE + ADD COLUMN 完成"
echo ""
note "下一步：pm2 reload edu-api（部署 V42 代码侧 InvoiceModule + Repository + Controller）"
note "灰度验证：抽样 INSERT/SELECT 测试 invoices 表 + partial UNIQUE 索引行为"
note "回退：见 migrations/V42__invoices_in_tenant_schema.sql 注释末段"
echo ""
