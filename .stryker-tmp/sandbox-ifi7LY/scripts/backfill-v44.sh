#!/bin/bash
# ============================================================
# backfill-v44.sh — V44 软删除 deleted_at 列全租户灌入脚本
#
# 来源：用户 2026-05-16 T12 软删除 V44 spec / R1 audit P0-3 修复
#
...
#   bash scripts/backfill-v44.sh
#
#   # 2. 真执行
#   bash scripts/backfill-v44.sh --apply
#
#   # 3. 只跑指定 tenant
#   bash scripts/backfill-v44.sh --apply --tenant-id=01abcd...
#
# 前置：
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V44__add_deleted_at_to_students_teachers_users.sql"
...
echo ""
echo "==============================================="
echo "  V44 deleted_at backfill (students/teachers/users)"
echo "==============================================="
echo ""
if [ "$APPLY" = false ]; then
...
while IFS='|' read -r TENANT_ID TENANT_NAME; do
  ...
  TMP_SQL=$(mktemp /tmp/v44-${TENANT_ID_LC}.XXXXXX.sql)
  ...
  if [ "$APPLY" = false ]; then
    info "[dry-run] would run V44 on ${TENANT_SCHEMA} (sql size: $(wc -c < "$TMP_SQL") bytes)"
    SUCCESS=$((SUCCESS + 1))
    rm -f "$TMP_SQL"
    continue
  fi
  ...
  if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" >/dev/null 2>&1; then
    ok "V44 applied to ${TENANT_SCHEMA}"
    SUCCESS=$((SUCCESS + 1))
  else
    fail "V44 failed on ${TENANT_SCHEMA}"
    # 再跑一次显示错误细节
...
ok "全部 tenant V44 backfill 完成"
echo ""
note "下一步：跑 scripts/full-test-prod.sh 验证 192 ALTER + 192 索引落地"
note "下一步：5/16 commit deploy ssh pdfserver 运行 bash scripts/backfill-v44.sh --apply"
echo ""
