...
describe('UserController.deactivate - Sprint X.2 (D6 JWT 黑名单)', () => {
  ...
  it('Redis 抛错 → fail-open, deactivate 仍成功', async () => {
    ...
    expect(result.user.status).toBe('停用');
    // refresh 用 ('b-user', userId) 撤销所有 refresh token
    expect(refreshToken.revokeAllBySubject).toHaveBeenCalledTimes(1);
    expect(refreshToken.revokeAllBySubject).toHaveBeenCalledWith(
      'b-user',
      targetUserId,
    );
    // audit_log 必落 'user.deactivated.jwt-revoked' (Redis 挂仍要 audit)
  ...
  it('refresh revoke 抛错 → fail-open, deactivate 仍成功', async () => {
    ...
    expect(result.user.status).toBe('停用');
    // Redis 写黑名单 key + audit 仍落（refresh fail-open 不阻塞其他副作用）
