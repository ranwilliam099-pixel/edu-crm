...
describe('RealWxPayClient (W2-T1)', () => {
  ...
  describe('onModuleInit', () => {
    it('WXPAY_MODE=mock → lazy-only（不加载配置）', () => {
      // 用 spy 验证 mock 模式确实跳过 loadConfig（不是「没抛错就 pass」）
      config.get.mockImplementation((k: string, def?: unknown) =>
        k === 'WXPAY_MODE' ? 'mock' : def,
      );
      client.resetConfigCache();
      const loadConfigSpy = jest.spyOn(
        client as unknown as { loadConfig: () => void },
        'loadConfig',
      );
      client.onModuleInit();
      // 关键断言：mock 模式 NEVER 调用 loadConfig（lazy-only）
      expect(loadConfigSpy).not.toHaveBeenCalled();
      // 且 cachedConfig 应仍为 null（resetConfigCache 后未 hydrate）
      expect(
        (client as unknown as { cachedConfig: unknown }).cachedConfig,
      ).toBeNull();
