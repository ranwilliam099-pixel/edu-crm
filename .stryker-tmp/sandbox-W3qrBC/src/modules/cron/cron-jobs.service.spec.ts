...
describe('CronJobsService - W3-1 收尾全局 cron 编排', () => {
  ...
  describe('monthlyRenewActiveSubscriptionsInDb - V23 真接 PG', () => {
    it('mock paymentExecutor=true → 续费成功 + persist', async () => {
      ...
      // 验证 sub + paymentOrder 都被持久化（防 silent skip）+ 参数关键字段
      expect(m.upsertSubscription).toHaveBeenCalledTimes(1);
      expect(m.upsertSubscription).toHaveBeenCalledWith(
      ...
      expect(m.insertPaymentOrder).toHaveBeenCalledTimes(1);
      expect(m.insertPaymentOrder).toHaveBeenCalledWith(
        expect.objectContaining({
          id: 'po01', // mock idGenerator 返回值
          parentId: 'par01',
        }),
      );
    });
