#!/bin/bash
# ============================================================
# backfill-v46.sh — V46 password_hash 列全租户灌入脚本
#
# 来源：
...
#   bash scripts/backfill-v46.sh
#
#   # 2. 真执行
#   bash scripts/backfill-v46.sh --apply
#
#   # 3. 只跑指定 tenant
#   bash scripts/backfill-v46.sh --apply --tenant-id=01abcd...
#
# 前置：
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V46__add_password_to_users.sql"
...
echo ""
echo "==============================================="
echo "  V46 password_hash backfill (users)"
echo "==============================================="
echo ""
if [ "$APPLY" = false ]; then
...
while IFS='|' read -r TENANT_ID TENANT_NAME; do
  ...
  TMP_SQL=$(mktemp /tmp/v46-${TENANT_ID_LC}.XXXXXX.sql)
  ...
  if [ "$APPLY" = false ]; then
    info "[dry-run] would run V46 on ${TENANT_SCHEMA} (sql size: $(wc -c < "$TMP_SQL") bytes)"
    SUCCESS=$((SUCCESS + 1))
    rm -f "$TMP_SQL"
    continue
  fi
  ...
  if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" >/dev/null 2>&1; then
    ok "V46 applied to ${TENANT_SCHEMA}"
    SUCCESS=$((SUCCESS + 1))
  else
    fail "V46 failed on ${TENANT_SCHEMA}"
...
ok "全部 tenant V46 backfill 完成"
echo ""
note "下一步：scripts/backfill-v47.sh（public.parents.status 切中文，仅 public schema 单次执行）"
echo ""
