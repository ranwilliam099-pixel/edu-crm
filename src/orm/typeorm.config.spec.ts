...
describe('createTypeOrmOptions (W1 BE-W1-1)', () => {
  ...
  it('returns postgres dialect', () => {
    const opts = createTypeOrmOptions(makeConfig({}));
  ...
  it('verbose logging in development, error-only in production', () => {
    const dev = createTypeOrmOptions(makeConfig({ NODE_ENV: 'development' }));
    expect(dev.logging).toEqual(['error', 'warn']);
    const prod = createTypeOrmOptions(makeConfig({ NODE_ENV: 'production' }));
