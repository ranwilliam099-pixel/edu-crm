 ...
 * 总 case 数: 21 对象 × 13 角色 = 273
 *
 * 与 prompt 数字差异:
 *   prompt 任务 B 说 "9 角色 × 18 对象 = 162"，本 spec 实际 13 × 21 = 273
 *   多 111 case 覆盖 2 平台 + 4 auxiliary 角色 = 6 × 18 = 108
...
describe('[RBAC L9 Batch C] 跨 tenant 强制 403 — 21 对象 × 13 角色 = 273 case', () => {
  ...
  describe('teacher_rating — cross-tenant body.tenantSchema mismatch', () => {
    it('admin → ForbiddenException (cross-tenant denied)', () => {
      expect(() => guard.canActivate(mkMismatchRequest(mkSelfUser('admin')))).toThrow(ForbiddenException);
    });
    it('boss → ForbiddenException (cross-tenant denied)', () => {
  ...
  describe('c_message — cross-tenant body.tenantSchema mismatch', () => {
    it('admin → ForbiddenException (cross-tenant denied)', () => {
      expect(() => guard.canActivate(mkMismatchRequest(mkSelfUser('admin')))).toThrow(ForbiddenException);
    });
    it('boss → ForbiddenException (cross-tenant denied)', () => {
