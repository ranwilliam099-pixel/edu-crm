...
describe('ScheduleService - V8 BE-V8-1 PD §3 + 条目 31/32 (Wave 11 academic 唯一)', () => {
  ...
  describe('createSchedule - service 层 RBAC 兜底（2026-05-30 SSOT §5.3 教务双层）', () => {
    const baseInput = {
      id: ULID32_SCH1,
      teacherId: ULID32_T1,
    ...
    // 2026-05-30 SSOT §5.3 line 426：academic_admin（教务主管）放行（service 兜底亦通过）
    it('callerRole=academic_admin（教务主管，拍板放行）→ 通过，createdByRole=academic_admin', () => {
      const { schedule } = service.createSchedule(
        {
          ...baseInput,
          currentUser: { ...academicUser, role: 'academic_admin' },
      ...
      expect(schedule.status).toBe('已排课');
      expect(schedule.createdByRole).toBe('academic_admin');
    });
