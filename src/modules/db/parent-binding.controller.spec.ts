...
describe('ParentBindingController.createParent (Sprint X.2 SSOT §12.5)', () => {
  ...
  it('phone 命中 C 端 parent → 复用 parent (一家长多孩子)', async () => {
    ...
    // 仍 insert binding — 复用 parent 但新建 binding 指向新 student
    expect(parentRepo.insertBinding).toHaveBeenCalledTimes(1);
    expect(parentRepo.insertBinding).toHaveBeenCalledWith(
      expect.objectContaining({
        parentId: ULID32_PARENT, // 复用既有 parent
        studentId: ULID32_STUDENT,
        relationship: 'father',
        tenantId: ULID32_T,
      }),
