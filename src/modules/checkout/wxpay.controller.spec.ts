...
describe('WxPayController', () => {
  ...
  describe('wxpayCallback', () => {
    ...
    it('refund SUCCESS → 调 handleRefundNotify', async () => {
      ...
      expect(r.code).toBe('SUCCESS');
      // 验证 (headers, rawBody, notifyBody) 三参 + headers 必含 V3 验签四件套
      expect(callback.handleRefundNotify).toHaveBeenCalledTimes(1);
      expect(callback.handleRefundNotify).toHaveBeenCalledWith(
        expect.objectContaining({
        ...
        expect.anything(), // rawBody (Buffer / string)
        body.notifyBody,
      );
    });
