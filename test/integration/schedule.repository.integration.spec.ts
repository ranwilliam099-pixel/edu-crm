...
describe('ScheduleRepository [integration, real PG, X1 schedule+consumption 联动]', () => {
  ...
  it('bulkUpsertFromRecurring 幂等：同 recurring + start_at 重复展开第 2 次 skipped', async () => {
    ...
    const bindingId = testUlid();
    // V8.1 schema: 先建 student_teacher_bindings（recurring_schedules.binding_id FK 必需）
    // 再建 recurring_schedules（by_day JSONB / start_minutes / duration_min / start_date / status='active'）
    await runInSchema(schema, async (client) => {
      await client.query(
        `INSERT INTO student_teacher_bindings
           (id, student_id, teacher_id, subject, status, bound_by_user_id)
         VALUES ($1, $2, $3, '数学', 'active', $4)`,
        [bindingId, studentId, teacherId, salesUserId],
      );
      await client.query(
        `INSERT INTO recurring_schedules
           (id, binding_id, student_id, teacher_id, by_day, start_minutes, duration_min,
            start_date, status, created_by_user_id, created_by_role)
         VALUES ($1, $2, $3, $4, $5::jsonb, $6, $7, $8, 'active', $9, 'academic')`,
        [
          recurringId,
          bindingId,
          studentId,
          teacherId,
          JSON.stringify(['MO', 'WE']),
          600, // 10:00 = 10*60
          60,
          '2026-07-01',
          salesUserId,
