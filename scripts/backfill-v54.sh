#!/bin/bash
# ============================================================
# backfill-v54.sh — V54 invoices.payment_method + invoices.paid_at 全租户 ADD
#
...
#       （参考 V33/V34/V35/V36/V37/V39 模式 — 与 V41 同型；V54 无 ts-node 数据 backfill）
#
# 与 V41 关键差异：
#   - V41 是「ADD COLUMN + UPDATE 加密数据」需 ts-node 单 tenant 数据回填
#   - V54 是「ALTER TABLE + ADD COLUMN DEFAULT FALSE」纯 schema 操作，无数据 backfill
...
#   - V54__invoices_payment_method_and_paid_at.sql 文件存在
#
# 幂等：
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V54__invoices_payment_method_and_paid_at.sql"
...
echo ""
echo "==============================================="
echo "  V54 invoices.payment_method + invoices.paid_at"
echo "==============================================="
echo ""
if [ "$APPLY" = false ]; then
...
if [ "$SKIP_BACKUP_CHECK" = true ]; then
  ...
  if [ "$BACKUP_FOUND" = false ]; then
    ...
    warn "V54 是 ALTER TABLE + ADD COLUMN（可逆），缺备份不阻塞，但建议先跑备份"
    if [ "$APPLY" = true ]; then
...
while IFS='|' read -r TENANT_ID TENANT_NAME; do
  ...
  if [ "$APPLY" = false ]; then
    ...
    if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" >/dev/null 2>&1; then
      ok "V54 applied to ${TENANT_SCHEMA}"
      SCHEMA_OK=$((SCHEMA_OK + 1))
    else
      fail "V54 failed on ${TENANT_SCHEMA}"
...
if [ $SCHEMA_FAILED -gt 0 ]; then
  fail "部分 tenant V54 失败（schema_failed=$SCHEMA_FAILED）"
  exit 1
fi

ok "全部 tenant V54 CREATE TABLE + ADD COLUMN 完成"
echo ""
note "下一步：pm2 reload edu-api（部署 V54 代码侧 InvoiceController.markPaid endpoint + repository.markPaid 5 步事务）"
note "灰度验证：抽样 INSERT/SELECT 测试 invoices 表 + partial UNIQUE 索引行为"
note "回退：见 migrations/V54__invoices_payment_method_and_paid_at.sql 注释末段"
echo ""
