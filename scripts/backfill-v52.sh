#!/bin/bash
# ============================================================
# backfill-v52.sh — V52 audit_log.actor_user_id + target_id: UUID → VARCHAR(32)
#
...
#   bash scripts/backfill-v52.sh
#
#   # 2. 真执行
#   bash scripts/backfill-v52.sh --apply
#
#   # 3. 只跑指定 tenant
#   bash scripts/backfill-v52.sh --apply --tenant-id=01abcd...
#
# 前置：
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V52__schedules_add_campus_id.sql"
...
echo ""
echo "==============================================="
echo "  V52 schedules.campus_id 加列 backfill"
echo "==============================================="
echo ""
if [ "$APPLY" = false ]; then
...
while IFS='|' read -r TENANT_ID TENANT_NAME; do
  ...
  TMP_SQL=$(mktemp /tmp/v52-${TENANT_ID_LC}.XXXXXX.sql)
  ...
  if [ "$APPLY" = false ]; then
    info "[dry-run] would run V52 on ${TENANT_SCHEMA} (sql size: $(wc -c < "$TMP_SQL") bytes)"
    SUCCESS=$((SUCCESS + 1))
    rm -f "$TMP_SQL"
    continue
  fi
  ...
  if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" >/dev/null 2>&1; then
    ok "V52 applied to ${TENANT_SCHEMA}"
    SUCCESS=$((SUCCESS + 1))
  else
    fail "V52 failed on ${TENANT_SCHEMA}"
    # 再跑一次显示错误细节
...
ok "全部 tenant V52 backfill 完成"
echo ""
note "下一步：跑 scripts/full-test-prod.sh 验证 64 ALTER × 2 列类型变换落地"
note "下一步：5/16 commit deploy ssh pdfserver 运行 bash scripts/backfill-v52.sh --apply"
echo ""
