#!/bin/bash
# ============================================================
# backfill-v35.sh — V35 teacher_showcase_meta 全租户灌入脚本
#
# 来源：用户 2026-05-11 Sprint C 第一刀（双轨数据基础设施）
#
# 模式：bash 循环 tenant_ids + sed __TENANT_SCHEMA__ + sudo -u postgres psql
#       （参考 V33/V34 在 64 tenants × 2 = 128/128 实战全过经验）
#
# 用法（生产服务器，由 main session 审完再触发）：
#   # 1. dry-run（默认）— 不真跑，只列将处理的 tenant
#   bash scripts/backfill-v35.sh
#
#   # 2. 真执行
#   bash scripts/backfill-v35.sh --apply
#
#   # 3. 只跑指定 tenant
#   bash scripts/backfill-v35.sh --apply --tenant-id=01abcd...
#
# 前置：
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V35__teacher_showcase_meta.sql"
...
echo ""
echo "==============================================="
echo "  V35 teacher_showcase_meta backfill"
echo "==============================================="
echo ""
if [ "$APPLY" = false ]; then
...
while IFS='|' read -r TENANT_ID TENANT_NAME; do
  ...
  TMP_SQL=$(mktemp /tmp/v35-${TENANT_ID_LC}.XXXXXX.sql)
  ...
  if [ "$APPLY" = false ]; then
    info "[dry-run] would run V35 on ${TENANT_SCHEMA} (sql size: $(wc -c < "$TMP_SQL") bytes)"
    SUCCESS=$((SUCCESS + 1))
    rm -f "$TMP_SQL"
    continue
  fi
  ...
  if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" >/dev/null 2>&1; then
    ok "V35 applied to ${TENANT_SCHEMA}"
    SUCCESS=$((SUCCESS + 1))
  else
    fail "V35 failed on ${TENANT_SCHEMA}"
    # 再跑一次显示错误细节
...
ok "全部 tenant V35 backfill 完成"
echo ""
note "下一步（C.2）：实现 teacher-showcase-meta.repository.ts + Controller PUT endpoint"
note "下一步（C.3）：V17 parent_recommendations.toggleDisplayed 时同步 meta.count"
echo ""
