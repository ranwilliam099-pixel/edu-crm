#!/bin/bash
# ============================================================
# backfill-v57.sh — V57 parent_communication 全租户 CREATE TABLE
#
# 来源：用户 2026-05-22 SSOT §3.4 Sprint Y P2 unreadConsultations 数据源
#
# 模式：bash 循环 tenant_ids + sed __TENANT_SCHEMA__ + sudo -u postgres psql
# 幂等：CREATE TABLE IF NOT EXISTS + CREATE INDEX IF NOT EXISTS
#
# 用法（生产服务器）：
#   bash scripts/backfill-v57.sh             # dry-run 默认
#   bash scripts/backfill-v57.sh --apply     # 真跑
# ============================================================
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V57__parent_communication.sql"
...
for tenant_id in $TENANTS; do
  ...
  if [ "$APPLY" = false ]; then
    printf "${C_YELLOW}DRY${C_RESET}   ${schema}: 将 CREATE TABLE parent_communication\n"
    continue
  fi

  TMP_SQL=$(mktemp /tmp/v57-${schema}.XXXXXX.sql)
  ...
  if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" > /dev/null 2>&1; then
    ok "${schema}: CREATE TABLE parent_communication"
    SUCCESS=$((SUCCESS + 1))
  else
