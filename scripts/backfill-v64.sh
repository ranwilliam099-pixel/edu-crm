#!/bin/bash
# ============================================================
# backfill-v64.sh — V64 试听课流程（Phase 4）全租户 schema migration
#   (CREATE TABLE trials + 4 indexes)
#
# 来源：../edu-mp-sandbox/docs/2026-06-01-业务链方案-试听激活分配反馈.md Phase 4（#9）
# 业务：销售发起试听 → 复用 Phase 3 校长分配规则派教务 → 教务排老师/开试听 → 转化/流失。
#
# 模式：bash 循环 tenant_ids + sed __TENANT_SCHEMA__ + sudo -u postgres psql
#       （完全仿 backfill-v63.sh；含 COS pre-DDL 备份探测红线）
#
# 内容：
#   1. CREATE TABLE trials（可逆 DROP TABLE）
#   2. 4 个索引（status / campus / assigned_academic 部分 / teacher_scheduled 部分）
#   均幂等（IF NOT EXISTS），无数据 backfill（trials 初始空）。
#   分配游标复用 V63 campus_assignment_config.rr_last_academic_id（不新增列，无 ALTER）。
#
# 用法（生产服务器，在 PG 主机上执行）：
#   # 1. dry-run（默认）— 列将处理的 tenant，不改库
#   bash scripts/backfill-v64.sh
#
#   # 2. 真执行
#   bash scripts/backfill-v64.sh --apply
#
#   # 3. 跳过备份检查（DANGEROUS，仅在确认 pg_dump 还原点已存在时用；主机无 coscmd 时常用）
#   bash scripts/backfill-v64.sh --apply --skip-backup-check
#
#   # 4. 只跑指定 tenant
#   bash scripts/backfill-v64.sh --apply --tenant-id=01abcd...
#
# 前置：
#   - 在 PG 主机执行（sudo -u postgres psql 本机连接）
#   - PG_DB env 默认 'edu'，可覆盖
#   - migrations/V64__trials.sql 文件存在
#
# 幂等：
#   - CREATE TABLE IF NOT EXISTS / CREATE INDEX IF NOT EXISTS（重跑无害）
#
# 出具：edu-server backend  2026-06-01（Phase 4 试听课流程）
# ============================================================
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V64__trials.sql"
...
echo ""
echo "==============================================="
echo "  V64 试听课流程（Phase 4）"
echo "==============================================="
echo ""
if [ "$APPLY" = false ]; then
  warn "DRY-RUN mode（默认）— 加 --apply 才会真执行"
else
  info "APPLY mode — 真执行（CREATE TABLE trials + 4 indexes）"
fi
...
if [ "$SKIP_BACKUP_CHECK" = true ]; then
  ...
  warn "请确认已有 pg_dump 还原点（V64 = CREATE TABLE，可逆，无数据 backfill，风险低）"
  ...
  if [ "$BACKUP_FOUND" = false ]; then
    ...
    warn "V64 是 CREATE TABLE（可逆），缺备份不阻塞，但建议先跑备份"
    if [ "$APPLY" = true ]; then
...
echo "==============================================="
echo "  开始逐租户 CREATE TABLE trials"
echo "==============================================="
...
while IFS='|' read -r TENANT_ID TENANT_NAME; do
  ...
  # ----- CREATE TABLE trials -----
  TMP_SQL=$(mktemp /tmp/v64-${TENANT_ID_LC}.XXXXXX.sql)
  ...
  if [ "$APPLY" = false ]; then
    info "[dry-run] would CREATE TABLE trials + 4 indexes on ${TENANT_SCHEMA} (sql size: $(wc -c < "$TMP_SQL") bytes)"
    SCHEMA_OK=$((SCHEMA_OK + 1))
  else
    if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" >/dev/null 2>&1; then
      ok "V64 applied to ${TENANT_SCHEMA}"
      SCHEMA_OK=$((SCHEMA_OK + 1))
    else
      fail "V64 failed on ${TENANT_SCHEMA}"
...
echo "==============================================="
echo ""
echo "  CREATE TABLE trials + 4 indexes:"
...
if [ $SCHEMA_FAILED -gt 0 ]; then
  fail "部分 tenant V64 失败（schema_failed=$SCHEMA_FAILED）"
  exit 1
fi

ok "全部 tenant V64 CREATE TABLE trials 完成"
echo ""
note "下一步：pm2 reload edu-api（部署 TrialAssignmentService + TrialController 试听全流程端点）"
note "灰度验证：销售 POST /db/trials 发起试听 → 校长开自动分配开关时自动派教务（status pending_teacher）；关时 pending_assign 校长手动派"
note "回退：DROP TABLE IF EXISTS <schema>.trials;（分配游标复用 V63 config，无额外回退）"
echo ""
