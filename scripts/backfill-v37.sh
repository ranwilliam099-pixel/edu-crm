#!/bin/bash
# ============================================================
# backfill-v37.sh — V37 monthly_aggregates.payroll_yuan DROP 全租户脚本
#
# 来源：用户 2026-05-11 Sprint A 第一刀（薪资业务下线 P0）
#
# 模式：bash 循环 tenant_ids + sed __TENANT_SCHEMA__ + sudo -u postgres psql
#       （参考 V33/V34/V35 在 64 tenants × N = 全过经验）
#
# ⚠️ 与 V33/V34/V35 关键差异（W4）：
#   - V33-V35 是「加列/加表」幂等操作，**重跑安全**
#   - V37 是「**DROP 列 + 数据永久丢失**」操作，重跑无害但首次跑前必须有备份
#   - 故 backfill 前置加入 W2 COS 备份探测（pre-DROP pg_dump 必须存在）
#
# 用法（生产服务器，由 main session 审完再触发）：
#   # 1. dry-run（默认）— 不真跑，只列将处理的 tenant + 备份检查
#   bash scripts/backfill-v37.sh
#
#   # 2. 真执行
#   bash scripts/backfill-v37.sh --apply
#
#   # 3. 跳过备份检查（DANGEROUS，仅紧急回退用）
#   bash scripts/backfill-v37.sh --apply --skip-backup-check
#
#   # 4. 只跑指定 tenant
#   bash scripts/backfill-v37.sh --apply --tenant-id=01abcd...
#
# 前置：
...
#     未配则只会 banner 警告，不阻塞
#
# 幂等：ALTER TABLE DROP COLUMN IF EXISTS（重跑无害但数据已丢失）
#
# 出具：研发负责人  2026-05-11
# ============================================================
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V37__drop_monthly_aggregates_payroll.sql"
...
# 继承 backfill-v35.sh 第 65-71 行 — 应用层守护参考 pg-pool.service.ts:69 /^tenant_[a-z0-9_]+$/
...
echo ""
echo "==============================================="
echo "  V37 DROP monthly_aggregates.payroll_yuan"
echo "==============================================="
echo ""
if [ "$APPLY" = false ]; then
  warn "DRY-RUN mode（默认）— 加 --apply 才会真执行"
else
  info "APPLY mode — 真执行（数据永久丢失）"
fi
...
# ===== W2 红线：pre-DROP COS 备份存在性前置检查 =====
echo "==============================================="
echo "  W2 红线：pre-DROP pg_dump 备份探测"
echo "==============================================="
echo ""
...
if [ "$SKIP_BACKUP_CHECK" = true ]; then
  ...
  if [ "$BACKUP_FOUND" = false ]; then
    fail "${C_BOLD}未找到今日/昨日 pg_dump 备份在 COS${C_RESET}"
    fail "DROP 列是不可逆操作！必须先确保最近 pg_dump 已上传 COS"
    fail "操作建议："
    fail "  1. ssh 上 PG 主机手动跑 scripts/pg_dump_backup.sh"
    fail "  2. 等 COS 同步完成"
    fail "  3. 重跑本脚本（或加 --skip-backup-check 强制继续，自负风险）"
    exit 1
  fi
else
  ...
  if [ "$APPLY" = true ]; then
    ...
    fail "请先确认备份，或加 --skip-backup-check 强制（自负风险）"
    exit 1
  fi
fi
echo ""
...
while IFS='|' read -r TENANT_ID TENANT_NAME; do
  ...
  TMP_SQL=$(mktemp /tmp/v37-${TENANT_ID_LC}.XXXXXX.sql)
  sed "s/__TENANT_SCHEMA__/${TENANT_SCHEMA}/g" "$MIGRATION_FILE" > "$TMP_SQL"

  if [ "$APPLY" = false ]; then
    info "[dry-run] would DROP payroll_yuan on ${TENANT_SCHEMA} (sql size: $(wc -c < "$TMP_SQL") bytes)"
    SUCCESS=$((SUCCESS + 1))
    rm -f "$TMP_SQL"
    continue
  fi
  ...
  if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" >/dev/null 2>&1; then
    ok "V37 applied to ${TENANT_SCHEMA}"
    SUCCESS=$((SUCCESS + 1))
  else
    fail "V37 failed on ${TENANT_SCHEMA}"
...
ok "全部 tenant V37 payroll_yuan DROP 完成"
echo ""
note "下一步：pm2 reload edu-api（部署 V37 代码侧 dashboard.repository 改造）"
note "回退（仅恢复列结构，**不恢复数值**）：见 migrations/V37__drop_monthly_aggregates_payroll.sql 注释末段"
echo ""
