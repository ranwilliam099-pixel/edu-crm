#!/bin/bash
# ============================================================
# backfill-v60.sh — V60 assessment_recipients 全租户 CREATE TABLE
#
# 来源：用户 2026-05-23 task #33 — assessment_recipients 接收方表
#
# 模式：bash 循环 tenant_ids + sed __TENANT_SCHEMA__ + sudo -u postgres psql
# 幂等：CREATE TABLE IF NOT EXISTS + CREATE INDEX IF NOT EXISTS
#
# 用法：
#   bash scripts/backfill-v60.sh             # dry-run
#   bash scripts/backfill-v60.sh --apply     # 真跑
# ============================================================
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V60__assessment_recipients_in_tenant_schema.sql"
...
for tenant_id in $TENANTS; do
  ...
  if [ "$APPLY" = false ]; then
    printf "${C_YELLOW}DRY${C_RESET}   ${schema}: 将 CREATE TABLE assessment_recipients\n"
    continue
  fi

  TMP_SQL=$(mktemp /tmp/v60-${schema}.XXXXXX.sql)
  ...
  if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" > /dev/null 2>&1; then
    ok "${schema}: CREATE TABLE assessment_recipients"
    SUCCESS=$((SUCCESS + 1))
  else
