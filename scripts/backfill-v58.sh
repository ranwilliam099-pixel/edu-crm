#!/bin/bash
# ============================================================
# backfill-v58.sh — V58 teacher_change_requests 全租户 CREATE TABLE
#
# 来源：用户 2026-05-22 SSOT §6.5 拍板「改老师 = 家长同意」
#
# 模式：bash 循环 tenant_ids + sed __TENANT_SCHEMA__ + sudo -u postgres psql
# 幂等：CREATE TABLE IF NOT EXISTS + CREATE INDEX IF NOT EXISTS
#
# 用法：
#   bash scripts/backfill-v58.sh             # dry-run
#   bash scripts/backfill-v58.sh --apply     # 真跑
# ============================================================
...
MIGRATION_FILE="${REPO_ROOT}/migrations/V58__teacher_change_requests.sql"
...
for tenant_id in $TENANTS; do
  ...
  if [ "$APPLY" = false ]; then
    printf "${C_YELLOW}DRY${C_RESET}   ${schema}: 将 CREATE TABLE teacher_change_requests\n"
    continue
  fi

  TMP_SQL=$(mktemp /tmp/v58-${schema}.XXXXXX.sql)
  ...
  if sudo -u "$PG_USER_OS" psql -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$TMP_SQL" > /dev/null 2>&1; then
    ok "${schema}: CREATE TABLE teacher_change_requests"
    SUCCESS=$((SUCCESS + 1))
  else
